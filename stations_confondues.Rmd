...
```{r}
data.train<-data[1:floor(0.85*nrow(data)),]
data.test<-data[(floor(0.85*nrow(data))+1):nrow(data),]
```
...
```
interprétations: 
  ¤ l'Acf diminue graduellement > très longue mémoire ou composante intégrée. moyenne mobile présente
  ¤ p=2 sur le graphe d'auto-régression. la valeur actuelle est influencée surtout par les 2 dernières valeures
...
mod.arima<-Arima(data.train$IQA_numeric, order=c(2,1,15),xreg = as.matrix(external_vars))
